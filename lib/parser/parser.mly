...
types:
  ...
  | type_array { $1 }
  | type_struct { $1 }
  | IDENT LT tys=separated_nonempty_list(COMMA, types) GT { mk_types $startpos (`Named $1) }
...
expr_function:
  ...
  | LT tys=separated_nonempty_list(COMMA, IDENT) GT LPAREN RPAREN b=block     
      { mk_expr $startpos (`Fn { ty=None; params=[]; block=b }) }
  | LT tys=separated_nonempty_list(COMMA, IDENT) GT LPAREN RPAREN ty=types b=block     
      { mk_expr $startpos (`Fn { ty=Some(ty); params=[]; block=b }) }
  | LT tys=separated_nonempty_list(COMMA, IDENT) GT LPAREN p=separated_nonempty_list(COMMA, function_param) RPAREN b=block     
      { mk_expr $startpos (`Fn { ty=None; params=p; block=b }) }
  | LT tys=separated_nonempty_list(COMMA, IDENT) GT LPAREN p=separated_nonempty_list(COMMA, function_param) RPAREN ty=types b=block     
      { mk_expr $startpos (`Fn { ty=Some(ty); params=p; block=b }) }
...
declaration_struct:
  ...
  | STRUCT name=IDENT LT tys=separated_nonempty_list(COMMA, IDENT) GT LBRACE option(newlines) RBRACE { mk_stmt $startpos (`Struct {fields=[]; name}) }
  | STRUCT name=IDENT LT tys=separated_nonempty_list(COMMA, IDENT) GT LBRACE option(newlines) fields=struct_fields option(newlines) RBRACE { mk_stmt $startpos (`Struct {fields; name}) }
struct_fields:
  | struct_field      { [ $1 ] }
...
expr_struct:
  ...
  | name=IDENT LT tys=separated_nonempty_list(COMMA, types) GT  LBRACE option(newlines) RBRACE         { mk_expr $startpos (`Struct {name=Some name; values=[]}) }
  | name=IDENT LT tys=separated_nonempty_list(COMMA, types) GT  LBRACE option(newlines) values=struct_values option(newlines) RBRACE         { mk_expr $startpos (`Struct {name=Some name; values}) }
struct_values:
  | struct_value                                    { [ $1 ] }
